...
resource "aws_iam_role" "secrets_role" {
  name = "${var.environment}-ridebooking-secrets-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            ...
            "${replace(aws_iam_openid_connect_provider.main.url, "https://", "")}:sub" = "system:serviceaccount:ridebooking:ridebooking-sa"
          }
        }
      }
    ]
